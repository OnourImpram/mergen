...
def _require_key(key: str) -> None:
    """Reject an empty key at the library boundary, not only at the CLI.

    The fail-closed rule (never produce or accept a token under an empty or guessable key) must
    hold for a direct importer of sign/verify too, not only for a caller who goes through main().
    """
    if not key:
        raise ValueError("key must be non-empty; set MERGEN_SIGNING_KEY to a real key")
...
def _cmd_sign(args: argparse.Namespace) -> int:
    key = _read_key()
    if key is None:
        print(f"error: set {_KEY_ENV} in the environment to a non-empty key", file=sys.stderr)
        return 2
...
def _cmd_verify(args: argparse.Namespace) -> int:
    ...
    if data is None:
        ...
        return 2
    digest = artifact_hash(data)
    if verify(digest, args.token, key):
