...
| `speckit.implement` | `/mergen.implement` | `speckit.implement` (preset override) | Wave-parallel execution from DAG. Isolated implementer context per task. Separate-context refute-biased verifier per task. TDD enforced via `test_task`. Re-queue with failure evidence on `pass: false`. Non-bypassable final verify gate. |
