...
| `core/hooks/verify_agent_hook.py` | An OPT-IN, default-off Stop hook that re-surfaces the recorded verification verdict at the end of a turn, framed as reinforcement, never as the gate. With `MERGEN_VERIFY_HOOK` unset it is a true no-op. When opted in it reads any committed `.specify/verification-report.json` read-only and surfaces the verdict plus the command to recompute. The CI verification step stays the enforcement; a fresh impacted-slice run is the named forward path | enable with `MERGEN_VERIFY_HOOK=1` and register as a Stop hook |
| `scripts/preaction_sign.py` | A pre-action authorization token bound to an artifact's exact bytes: `sign` returns an HMAC of the artifact hash under a local key (`MERGEN_SIGNING_KEY`, read from the environment, never printed), `verify` checks it in constant time. Because the token is a function of the bytes, it authorizes that one diff or report and cannot be copied onto another, unlike the plaintext `Governor-Ack` line. Honest scope: a shared-secret MAC, not a public-key signature. Keyless cosign or Sigstore is the named external path for public-verifiable identity | `mergen sign sign --artifact change.diff` then `mergen sign verify --artifact change.diff --token <hex>` |
| `scripts/adapter_sdk.py` | The Adapter SDK: per-host capability manifests (`core/adapters/*.json`) declare, as data, exactly what each render target can and cannot do. `require_capability` is the enforcement primitive a renderer calls to refuse a capability the host's manifest denies (the built-in renderers do not wire it yet, so it guards any new or external renderer and backs the matrix), `validate` checks each manifest is complete and named after its file, and the host capability matrix in [`CAPABILITIES.md`](CAPABILITIES.md) is generated from the manifests rather than hand-maintained | `python scripts/adapter_sdk.py matrix` or `mergen adapter matrix` |
| `eval/benchmark.py` | Deterministic phantom-detection benchmark, no LLM | `python eval/benchmark.py --gate` |
| `eval/evalops.py` | EvalOps: scores the shipped harness over the committed labelled corpus in `eval/corpus/` (growable a scenario at a time), records the score to a trend history, and gates on both the absolute floor and a catch-rate regression below the best the corpus ever scored. Deterministic surface only, no live model or tool chain | `python eval/evalops.py gate --history eval/evalops-trend.jsonl` |
| `eval/live_chain_eval.py` | The live-chain eval skeleton for the two north-star metrics a model-free run cannot measure (parallel speedup, over-build). It states the protocol as data and refuses to emit any number without a live runner, so it never fabricates a measurement. The real run is the operator's live-environment job, named in [`LIVE-CHAIN-EVAL.md`](LIVE-CHAIN-EVAL.md), not faked. Packaging readiness is in [`PACKAGING.md`](PACKAGING.md) | `python eval/live_chain_eval.py protocol` |
